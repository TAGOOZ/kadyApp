-- 0033_fix_availability_guard.sql — prod hotfix: disabled items still orderable
-- Adds `is_available` check to order_intake_pipeline (0001:204 column exists,
-- 0030/0032 never checked it). Single predicate, no DAG change.
-- Keeps 0032 hash/size fixes intact, only patches the price lookup.
...
begin
  ...
  if new.items is not null and jsonb_typeof(new.items) = 'array' then
    ...
    loop
      ...
      select price_egp, is_available into v_price, v_is_available from public.menu_items where id = v_id;
      if v_price is null then
        raise exception 'validate_order_pricing: menu item % not found', v_id using errcode = '22023';
      end if;
      if coalesce(v_is_available, true) = false then
        raise exception 'validate_order_pricing: menu item % is unavailable', v_id using errcode = '22023';
      end if;
...
comment on function public.order_intake_pipeline() is
  '0033: 0032 + is_available guard (select price_egp, is_available ... raise 22023 if unavailable). Keeps iced-espresso drink set and btrim/hash/size clamp.';

-- Verify trigger still exists and function has guard
do $$
declare vdef text := pg_get_functiondef((select oid from pg_proc where proname='order_intake_pipeline' limit 1));
begin
  if position('is_available' in vdef) = 0 then
    raise exception '0033: is_available guard missing from pipeline';
  end if;
  if position('is_available' in lower(vdef)) = 0 then
    raise exception '0033: guard not found';
  end if;
end;
$$;
